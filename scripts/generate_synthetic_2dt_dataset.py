"""Generate a synthetic example 2D+t dataset following the fUSI-BIDS v0.0.9 draft.

fUSI-BIDS draft link: https://docs.google.com/document/d/1W3z01mf1E8cfg_OY7ZGqeUeOKv659jCHQBXavtmT-T8/edit?usp=sharing"""
...
dataset_description = {
    "Name": f"fUS-BIDS v{fusi_bids_version} example dataset",
    "BIDSVersion": "1.9.0",
    "DatasetType": "raw",
...
readme = f"""# Example fUS-BIDS v{fusi_bids_version} dataset

This dataset provides an example for version {fusi_bids_version} of the [fUS-BIDS
...
tasks_json = {
    ...
    "stim": {
        ...
        "VolumeTiming": np.arange(0, 630, 0.4).tolist(),
    },
}
...
for subject_index in range(1, 11):
    ...
    for session in sessions:
        ...
        for task, task_json in tasks_json.items():
            for acq_value in acq_values:
                fusi_scan_path = fusi_path / (
                    f"sub-{subject_index:02d}_ses-{session}_task-{task}"
                    f"_acq-{acq_value}_pwd.nii.gz"
                )
                fusi_scan_path.touch()
                scan_paths_in_session.append(
                    str(fusi_scan_path.relative_to(session_path))
                )
                ...
                with open(sidecar_json_path, "w") as sidecar_json_file:
                    sidecar_json_file.write(json.dumps(task_json, indent=2))
